import { defineComponent, unref, useSSRContext, toRef, isRef, toRaw, openBlock, createElementBlock, normalizeStyle, normalizeClass, createBlock, resolveDynamicComponent, mergeProps, withCtx, createTextVNode, toDisplayString, resolveComponent, createCommentVNode, createElementVNode, Fragment, toHandlers, withModifiers, TransitionGroup, renderSlot, renderList, createVNode, getCurrentInstance, inject } from 'vue';
import { u as useHead, a as useNuxtApp } from './server.mjs';
...
const _sfc_main = {
  ...
  setup(__props) {
    useToast();
    const numbers = useState("nums", () => 10);
    useHead({
      title: "product page",
      meta: [
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<!--[--><p>product page</p><button type="button">showToast</button><button>change</button><p>${ssrInterpolate(unref(numbers))}</p><button type="button">add</button><!--]-->`);
    };
  }
};
...
//# sourceMappingURL=product-v1U9nhzW.mjs.map
